...
public abstract class BaseParser<V> extends BaseActions<V> {
	...
	/**
	 * Matches the {@link org.parboiled.support.Chars#EOI} (end of input) character.
	 */
	public static final Rule EOI = new CharMatcher(Chars.EOI);

	/**
	 * Matches the special {@link org.parboiled.support.Chars#INDENT} character produces by the org.parboiled.buffers.IndentDedentInputBuffer
	 */
	public static final Rule INDENT = new CharMatcher(Chars.INDENT);

	/**
	 * Matches the special {@link org.parboiled.support.Chars#DEDENT} character produces by the org.parboiled.buffers.IndentDedentInputBuffer
	 */
	...
	/**
	 * Creates a new instance of this parsers class using the no-arg constructor. If no no-arg constructor exists this method will fail with a java.lang.NoSuchMethodError. Using this method is faster than using {@link org.parboiled.Parboiled#createParser(Class, Object...)} for creating new parser instances since
	...
	/**
	 * Explicitly creates a rule matching the given character. Normally you can just specify the character literal directly in you rule description. However, if you don't want to go through {@link #fromCharLiteral(char)}, e.g. because you redefined it, you can also use this wrapper.
	 * <p>
	 * Note: This methods carries a {@link org.parboiled.annotations.Cached} annotation, which means that multiple invocations with the same argument will yield the same rule instance.
	 * </p>
	 *
	 * @param c
	 *            the char to match
	 ...
	 * <p>
	 * Note: This methods carries a {@link org.parboiled.annotations.Cached} annotation, which means that multiple invocations with the same arguments will yield the same rule instance.
	 * </p>
	 *
	 * @param cLow
	 *            the start char of the range (inclusively)
	 * @param cHigh
	...
	/**
	 * Explicitly creates a rule matching the given string. Normally you can just specify the string literal directly in you rule description. However, if you want to not go through {@link #fromStringLiteral(String)}, e.g. because you redefined it, you can also use this wrapper.
	 * <p>
	 * Note: This methods provides caching, which means that multiple invocations with the same argument will yield the same rule instance.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Creates a new rule that acts as a syntactic predicate, i.e. tests the given sub rule against the current input position without actually matching any characters. Succeeds if the sub rule succeeds and fails if the sub rule rails. Since this rule does not actually consume any input it will
	 * never create a parse tree node.
	 * </p>
	 * <p>
	 * Also it carries a {@link org.parboiled.annotations.SuppressNode} annotation, which means all sub nodes will also never create a parse tree node. This can be important for actions contained in sub rules of this rule that otherwise expect the presence of certain parse tree structures in their context. Also see
	 * {@link org.parboiled.annotations.SkipActionsInPredicates}
	 * </p>
	 * <p>
	 * Note: This methods carries a {@link org.parboiled.annotations.Cached} annotation, which means that multiple invocations with the same argument will yield the same rule instance.
	 * </p>
	...
	/**
	 * <p>
	 * Creates a new rule that acts as a syntactic predicate, i.e. tests the sequence of the given sub rule against the current input position without actually matching any characters. Succeeds if the sub sequence succeeds and fails if the sub sequence rails. Since this rule does not actually
	 * consume any input it will never create a parse tree node.
	 * </p>
	 * <p>
	 * Also it carries a {@link org.parboiled.annotations.SuppressNode} annotation, which means all sub nodes will also never create a parse tree node. This can be important for actions contained in sub rules of this rule that otherwise expect the presence of certain parse tree structures in their context. Also see
	 * {@link org.parboiled.annotations.SkipActionsInPredicates}
	 ...
	 * will never create a parse tree node.
	 * </p>
	 * <p>
	 * Also it carries a {@link org.parboiled.annotations.SuppressNode} annotation, which means all sub nodes will also never create a parse tree node. This can be important for actions contained in sub rules of this rule that otherwise expect the presence of certain parse tree structures in their context. Also see
	 * {@link org.parboiled.annotations.SkipActionsInPredicates}
	...
	/**
	 * <p>
	 * Creates a new rule that acts as an inverse syntactic predicate, i.e. tests the sequence of the given sub rules against the current input position without actually matching any characters. Succeeds if the sub sequence fails and fails if the sub sequence succeeds. Since this rule does not
	 * actually consume any input it will never create a parse tree node.
	 * </p>
	 * <p>
	 * Also it carries a {@link org.parboiled.annotations.SuppressNode} annotation, which means all sub nodes will also never create a parse tree node. This can be important for actions contained in sub rules of this rule that otherwise expect the presence of certain parse tree structures in their context. Also see
	 * {@link org.parboiled.annotations.SkipActionsInPredicates}
	 ...
	 * @param rule
	 *            the sub rule to match repeatedly.
	 * @return a new rule
	 */
	@Cached
	@DontLabel
	 ...
	 *            the sub rule to match repeatedly.
	 * @param separator
	 *            the separator to match, if null the individual sub rules will be matched without separator.
	 * @return a new rule
	 */
	@Cached
	@DontLabel
	...
	/**
	 * Explicitly marks the wrapped expression as an action expression. parboiled transforms the wrapped expression into an {@link org.parboiled.Action} instance during parser construction.
	 *
	 * @param expression
