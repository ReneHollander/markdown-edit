...
/**
 * A {@link org.parboiled.parserunners.ParseRunner} implementation that properly reports the first {@link org.parboiled.errors.InvalidInputError} if the input does not conform to the rule grammar. It performs exactly as the {@link org.parboiled.parserunners.BasicParseRunner} on valid input, however, on invalid input two more parsing runs are initiated: one for recording
 * the first parse error and one for collecting the error report information.
 */
public class ReportingParseRunner<V> extends AbstractParseRunner<V> {

	/**
	 * Create a new ReportingParseRunner instance with the given rule and input text and returns the result of its {@link #run(String)} method invocation.
	 *
	 * @param rule
	...
	protected ParsingResult<V> runLocatingMatch(InputBuffer inputBuffer) {
		ParseRunner<V> locatingRunner = new ErrorLocatingParseRunner<V>(getRootMatcher()).withValueStack(getValueStack());
		return locatingRunner.run(inputBuffer);
	}

	protected ParsingResult<V> runReportingMatch(InputBuffer inputBuffer, int errorIndex) {
		ParseRunner<V> reportingRunner = new ErrorReportingParseRunner<V>(getRootMatcher(), errorIndex).withParseErrors(getParseErrors()).withValueStack(getValueStack());
