 ...
 * <p>
 * This InputBuffer collapses all space and tab characters at the beginning of a text line into either nothing (if the line has the same indentation level as the previous line), a special {@link Chars#INDENT} character (if the line has a greater indentation level than the previous line) or one or
 * more {@link Chars#DEDENT} characters (if the line has a lower indentation level than the previous line).
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * This means that the highest index of this InputBuffer is probably smaller than that of the original input text buffer, since all line indentations and blank lines have been collapsed. However, the implementation will make sure that {@link #getPosition(int)}, {@link #extract(int, int)}, etc. will
 * work as expected and always return the "correct" result from the underlying, original input buffer.
 * </p>
 * <p>
...
public class IndentDedentInputBuffer implements InputBuffer {
	 ...
	 * @param strict
	 *            signals whether the buffer should throw an {@link org.parboiled.errors.IllegalIndentationException} on "semi-dedents", if false the buffer silently accepts these
	 ...
	 *            signals whether the buffer should swallow empty lines
	 * @throws org.parboiled.errors.IllegalIndentationException
	 *             if the input contains illegal indentations and the strict flag is set
	 */
