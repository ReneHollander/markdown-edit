...
public abstract class AbstractParseRunner<V> implements ParseRunner<V> {
	...
	protected MatcherContext<V> createRootContext(InputBuffer inputBuffer, MatchHandler matchHandler, boolean fastStringMatching) {
		return new MatcherContext<V>(inputBuffer, getValueStack(), getParseErrors(), matchHandler, rootMatcher, fastStringMatching);
	}

	protected ParsingResult<V> createParsingResult(boolean matched, MatcherContext<V> rootContext) {
		return new ParsingResult<V>(matched, rootContext.getNode(), getValueStack(), getParseErrors(), rootContext.getInputBuffer());
	}
}
