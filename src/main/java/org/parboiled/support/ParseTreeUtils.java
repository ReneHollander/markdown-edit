...
public final class ParseTreeUtils {
	 ...
	 * </p>
	 * <p>
	 * The path is a '/' separated list of node label prefixes describing the ancestor chain of the node to look for relative to the given parent node. If there are several nodes that match the given path the method returns the first one unless the respective path segments has the special prefix
	 * "last:". In this case the last matching node is returned.
	 * <p>
	 * <b>Example:</b> "per/last:so/fix" will return the first node, whose label starts with "fix" under the last node, whose label starts with "so" under the first node, whose label starts with "per".
	 * </p>
	 ...
	 *
	 * @param parents
	 *            the parent Nodes to look through
	 * @param path
	 *            the path to the Node being searched for
	 * @return the Node if found or null if not found
	 */
	 ...
	 *            the parent Node
	 * @param path
	 *            the path to the Nodes being searched for
	 * @param collection
	...
	public static <V, C extends Collection<Node<V>>> C collectNodesByPath(Node<V> parent, String path, C collection) {
		...
		return parent != null && hasChildren(parent) ? collectNodesByPath(parent.getChildren(), path, collection) : collection;
	}
	...
	public static <V, C extends Collection<Node<V>>> C collectNodes(Node<V> parent, Predicate<Node<V>> predicate, C collection) {
		checkArgNotNull(predicate, "predicate");
		checkArgNotNull(collection, "collection");
		return parent != null && hasChildren(parent) ? collectNodes(parent.getChildren(), predicate, collection) : collection;
	}
	...
	/**
	 * Creates a readable string represenation of the parse tree in the given {@link org.parboiled.support.ParsingResult} object.
	 *
	 * @param parsingResult
	...
	/**
	 * Creates a readable string represenation of the parse tree in thee given {@link org.parboiled.support.ParsingResult} object. The given filter predicate determines whether a particular node (incl. its subtree) is printed or not.
	 *
	 * @param parsingResult
	 *            the parsing result containing the parse tree
	 * @param nodeFilter
	 *            the predicate selecting the nodes to print
	 * @param subTreeFilter
	...
	public static <V> String printNodeTree(ParsingResult<V> parsingResult, Predicate<Node<V>> nodeFilter, Predicate<Node<V>> subTreeFilter) {
		...
		return printTree(parsingResult.parseTreeRoot, new NodeFormatter<V>(parsingResult.inputBuffer), nodeFilter, subTreeFilter);
	}
