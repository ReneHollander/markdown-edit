...
/**
 * Inserts code for wrapping the created rule into a VarFramingMatcher if the method contains local variables assignable to {@link org.parboiled.support.Var}.
 */
class VarFramingGenerator implements RuleMethodProcessor {
	...
	public void process(ParserClassNode classNode, RuleMethod method) throws Exception {
		...
		instructions.insertBefore(ret, new MethodInsnNode(INVOKESPECIAL, VAR_FRAMING_MATCHER.getInternalName(), "<init>", '(' + RULE_DESC + '[' + VAR_DESC + ")V"));
	...
	private void createVarFieldArray(RuleMethod method, InsnList instructions, AbstractInsnNode ret) {
		...
		for (int i = 0; i < count; i++) {
			...
			instructions.insertBefore(ret, new MethodInsnNode(INVOKEVIRTUAL, VAR.getInternalName(), "setName", "(Ljava/lang/String;)V"));
			// stack: <array> :: <array> :: <index> :: <var>
