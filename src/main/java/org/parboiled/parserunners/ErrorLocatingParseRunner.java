...
/**
 * A {@link org.parboiled.parserunners.ParseRunner} implementation that creates a simple {@link org.parboiled.errors.BasicParseError} for the first error found in the input and adds it to the list of ParseErrors. It never causes the parser to perform more than one parsing run and is rarely used directly. Instead its functionality is relied upon
 * by the {@link org.parboiled.parserunners.ReportingParseRunner} and {@link org.parboiled.parserunners.RecoveringParseRunner} classes.
 */
public class ErrorLocatingParseRunner<V> extends AbstractParseRunner<V> implements MatchHandler {
	...
	private boolean notTestNot(MatcherContext context) {
		return !(context.getMatcher() instanceof TestNotMatcher) && (context.getParent() == null || notTestNot(context.getParent()));
	}
}