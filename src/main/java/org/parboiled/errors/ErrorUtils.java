...
public final class ErrorUtils {
	 ...
	 *
	 * @param path
	 *            the path to the failed matcher
	 * @param errorIndex
	...
	public static String printParseError(ParseError error, Formatter<InvalidInputError> formatter) {
		...
		return printErrorMessage("%s (line %s, pos %s):", message, error.getStartIndex(), error.getEndIndex(), error.getInputBuffer());
	}
	 ...
	 *
	 * @param format
	 *            the format string, must include three placeholders for a string (the error message) and two integers (the error line / column respectively)
	 * @param errorMessage
	 *            the error message
	 * @param errorIndex
	 ...
	 *            the error message
	 * @param startIndex
	 *            the start location of the error as an index into the inputBuffer
	 * @param endIndex
