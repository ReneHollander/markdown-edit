 ...
 *
 * Construct this using @{link PegdownPlugins#builder}, and then passing in either the Java plugin classes, or precompiled rules (for greater control, or if using Scala rules).
 */
public class PegDownPlugins {
	...
	public static class Builder {
		...
		/**
		 * Add a plugin parser. This should either implement {@link InlinePluginParser} or {@link BlockPluginParser}, or both. The parser will be enhanced by parboiled before its rules are extracted and registered here.
		 *
		 * @param pluginParser
		...
		public PegDownPlugins build() {
			return new PegDownPlugins(inlinePluginRules.toArray(new Rule[0]), blockPluginRules.toArray(new Rule[0]), specialChars.toArray(new Character[0]));
		}
	}
}
