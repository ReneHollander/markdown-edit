...
public interface Context<V> {
	...
	/**
	 * Returns the {@link org.parboiled.support.MatcherPath} to the currently running matcher.
	 *
	 * @return the path
	 */
	MatcherPath getPath();
	 ...
	 * </p>
	 * <p>
	 * Fast string matching "short-circuits" the default practice of treating string rules as simple Sequence of character rules. When fast string matching is enabled strings are matched at once, without relying on inner CharacterMatchers. Even though this can lead to significant increases of
	 * parsing performance it does not play well with error reporting and recovery, which relies on character level matches. Therefore the {@link org.parboiled.parserunners.ReportingParseRunner} and {@link org.parboiled.parserunners.RecoveringParseRunner} implementations only enable fast string
	 * matching during their basic first parsing run and disable it once the input has proven to contain errors.
	 * </p>
	 *
