...
/**
 * A ValueStack is a stack implementation for parser values. The current state of the stack can be saved and restored with the methods {@link #takeSnapshot()} and {@link #restoreSnapshot(Object)} ()}, whose implementations should be super efficient since they are being used extensively during a
 * parsing run. A ValueStack also serves as an Iterable over the current stack values (the values are being provided with the last value (on top of the stack) first).
 *
 * @param <V>
...
public interface ValueStack<V> extends Iterable<V> {
	...
	/**
	 * Restores the stack state as previously returned by {@link #takeSnapshot()}. This cost of running this operation is negligible and independent from the size of the stack.
	 *
	 * @param snapshot
	 *            a snapshot object previously returned by {@link #takeSnapshot()}
	 */
