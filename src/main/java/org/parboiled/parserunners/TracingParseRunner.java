...
/**
 * A {@link ParseRunner} implementation used for debugging purposes. It exhibits the same behavior as the {@link ReportingParseRunner} but collects debugging information as to which rules did match and which didn't.
 */
public class TracingParseRunner<V> extends ReportingParseRunner<V> implements MatchHandler {
	...
	/**
	 * Attaches the given filter to this TracingParseRunner instance. The given filter is used to select the matchers to print tracing statements for. NOTE: The given filter must be of type Predicate<Tuple2<Context<?>, Boolean>>. The reason this type is not directly specified in the constructors
	 * signature is that this would make predicate expressions using the {@link org.parboiled.common.Predicates} operations and the predefined predicate constructors in {@link org.parboiled.support.Filters} much more cumbersome to write (due to Java limited type parameters inference logic you would have to explicitly
	 * state the type parameters in many places).
	 *
	 * @param filter
	 *            the matcher filter selecting the matchers to print tracing statements for. Must be of type Predicate<Tuple2<Context<?>, Boolean>>.
	...
	private void print(MatcherContext<?> context, boolean matched) {
		...
		getLog().receive(", " + (matched ? "matched" : "failed") + ", cursor at " + pos.line + ':' + pos.column + " after \"" + line.substring(0, Math.min(line.length(), pos.column - 1)) + "\"\n");
		lastPath = path;
	}
}
