...
class BodyWithSuperCallReplacer implements RuleMethodProcessor {
	...
	public boolean appliesTo(ParserClassNode classNode, RuleMethod method) {
		...
		return !method.isBodyRewritten() && method.getOwnerClass() == classNode.getParentClass() && method.getLocalVarVariables() == null; // if we have local variables we need to create a VarFramingMatcher
	...
	public void process(ParserClassNode classNode, RuleMethod method) throws Exception {
		...
		method.instructions.add(new MethodInsnNode(INVOKESPECIAL, classNode.getParentType().getInternalName(), method.name, method.desc));
