...
/**
 * A {@link org.parboiled.matchervisitors.MatcherVisitor} determining whether a matcher can legally succeed with an empty match.
 */
public class CanMatchEmptyVisitor implements MatcherVisitor<Boolean> {
	...
	public Boolean visit(OneOrMoreMatcher matcher) {
		Checks.ensure(!matcher.subMatcher.accept(this), "Rule '%s' must not allow empty matches as sub-rule of an OneOrMore-rule", matcher.subMatcher);
		return false;
	}
	...
	public Boolean visit(ZeroOrMoreMatcher matcher) {
		Checks.ensure(!matcher.subMatcher.accept(this), "Rule '%s' must not allow empty matches as sub-rule of an ZeroOrMore-rule", matcher.subMatcher);
		return true;
	}
