...
/**
 * The most basic of all {@link org.parboiled.parserunners.ParseRunner} implementations. It runs a rule against a given input text and builds a corresponding {@link org.parboiled.support.ParsingResult} instance. However, it does not report any parse errors nor recover from them. Instead it simply marks the ParsingResult as "unmatched" if the input
 * is not valid with regard to the rule grammar. It never causes the parser to perform more than one parsing run and is the fastest way to determine whether a given input conforms to the rule grammar.
 */
public class BasicParseRunner<V> extends AbstractParseRunner<V> implements MatchHandler {

	/**
	 * Create a new BasicParseRunner instance with the given rule and input text and returns the result of its {@link #run(String)} method invocation.
	 *
	 * @param rule
