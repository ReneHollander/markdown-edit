 ...
 * </p>
 * <p>
 * The parsing process works as following:</br> After the rule tree (which is in fact a directed and potentially even cyclic graph of {@link org.parboiled.matchers.Matcher} instances) has been created a root MatcherContext is instantiated for the root rule (Matcher). A subsequent call to {@link #runMatcher()} starts the
 * parsing process.
 * </p>
 * <p>
 * The MatcherContext delegates to a given {@link org.parboiled.MatchHandler} to call {@link org.parboiled.matchers.Matcher#match(org.parboiled.MatcherContext)}, passing itself to the Matcher which executes its logic, potentially calling sub matchers. For each sub matcher the matcher creates/initializes a subcontext with
 * {@link org.parboiled.matchers.Matcher#getSubContext(org.parboiled.MatcherContext)} and then calls {@link #runMatcher()} on it.
 * </p>
 * <p>
...
public class MatcherContext<V> implements Context<V> {
	 ...
	 *
	 * @param inputBuffer
	 *            the InputBuffer for the parsing run
	 * @param valueStack
	 *            the ValueStack instance to use for the parsing run
	 * @param parseErrors
	 *            the parse error list to create ParseError objects in
	 * @param matchHandler
	 *            the MatcherHandler to use for the parsing run
	 * @param matcher
	 *            the root matcher
	 * @param fastStringMatching
	 *            <p>
	 *            Fast string matching "short-circuits" the default practice of treating string rules as simple Sequence of character rules. When fast string matching is enabled strings are matched at once, without relying on inner CharacterMatchers. Even though this can lead to significant
	 *            increases of parsing performance it does not play well with error reporting and recovery, which relies on character level matches. Therefore the {@link org.parboiled.parserunners.ReportingParseRunner} and {@link org.parboiled.parserunners.RecoveringParseRunner} implementations only
	 *            enable fast string matching during their basic first parsing run and disable it once the input has proven to contain errors.
	 *            </p>
	 */
	public MatcherContext(InputBuffer inputBuffer, ValueStack<V> valueStack, List<ParseError> parseErrors, MatchHandler matchHandler, Matcher matcher, boolean fastStringMatching) {
		this(checkArgNotNull(inputBuffer, "inputBuffer"), checkArgNotNull(valueStack, "valueStack"), checkArgNotNull(parseErrors, "parseErrors"), checkArgNotNull(matchHandler, "matchHandler"), null, 0, fastStringMatching, new HashSet<MatcherPosition>());
	...
	public MatcherPath getPath() {
		if (path == null) {
			path = new MatcherPath(new MatcherPath.Element(matcher, startIndex, level), parent != null ? parent.getPath() : null);
		}
		return path;
	}
	...
	public boolean inPredicate() {
		return matcher instanceof TestMatcher || matcher instanceof TestNotMatcher || parent != null && parent.inPredicate();
	}
	...
	private void checkActionContext() {
		...
		Checks.ensure(unwrap(matcher) instanceof SequenceMatcher && intTag > 0 && subContext.matcher instanceof ActionMatcher, "Illegal call to getMatch(), getMatchStartIndex(), getMatchEndIndex() or getMatchRange(), " + "only valid in Sequence rule actions that are not in first position");
	}
	...
	public void createNode() {
		if (!nodeSuppressed) {
			node = new NodeImpl<V>(matcher, getSubNodes(), startIndex, currentIndex, valueStack.isEmpty() ? null : valueStack.peek(), hasError);
			if (parent != null) {
	...
	public final MatcherContext<V> getBasicSubContext() {
		if (subContext == null) {
			// init new level
			subContext = new MatcherContext<V>(inputBuffer, valueStack, parseErrors, matchHandler, this, level + 1, fastStringMatching, memoizedMismatches);
		} else {
	...
	public boolean runMatcher() {
		try {
			...
			throw new ParserRuntimeException(e, printParseError(new BasicParseError(inputBuffer, currentIndex, StringUtils.escape(String.format("Error while parsing %s '%s' at input position", matcher instanceof ActionMatcher ? "action" : "rule", getPath())))) + '\n' + e);
		}
	}
}
