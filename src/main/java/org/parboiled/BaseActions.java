...
public abstract class BaseActions<V> implements ContextAware<V> {
	...
	/**
	 * Creates a new {@link org.parboiled.support.IndexRange} instance covering the input text matched by the rule immediately preceding the action expression that is currently being evaluated. This call can only be used in actions that are part of a Sequence rule and are not at first position in this Sequence.
	 * 
	...
	/**
	 * <p>
	 * Returns the input text matched by the rule immediately preceding the action expression that is currently being evaluated. If the matched input text is empty the given default string is returned. This call can only be used in actions that are part of a Sequence rule and are not at first
	 * position in this Sequence.
	 * </p>
	 *
	 * @param defaultString
	 *            the default string to return if the matched input text is empty
	 ...
	 * </p>
	 *
	 * @return the first input char of the input text matched by the immediately preceding subrule or null, if the previous rule matched nothing
	 */
	 ...
	 * </p>
	 *
	 * @return the end index of the context immediately preceding current action, i.e. the index of the character immediately following the last matched character
	 */
	public int matchEnd() {
		check();
	...
	/**
	 * <p>
	 * Returns the current position in the underlying {@link org.parboiled.buffers.InputBuffer} as a {@link org.parboiled.support.Position} instance.
	 * </p>
	 * 
	 ...
	 *
	 * @param down
	 *            the number of elements to skip before removing the value (0 being equivalent to drop())
	 * @return true
	 * @throws IllegalArgumentException
	 *             if the stack does not contain enough elements to perform this operation
	 */
	 ...
	 *
	 * @param down
	 *            the number of elements to skip before replacing the value (0 being equivalent to poke(value))
	 * @param value
	...
	/**
	 * Returns true if the current rule is running somewhere underneath a Test/TestNot rule. Useful for example for making sure actions are not run inside of a predicate evaluation: <code>
	 * return Sequence(
	 * ...,
	...
	/**
	 * Determines whether the current rule or a sub rule has recorded a parse error. Useful for example for making sure actions are not run on erroneous input: <code>
	 * return Sequence(
	 * ...,
	...
	private void check() {
		Checks.ensure(context != null && context.getMatcher() != null, "Illegal rule definition: Unwrapped action expression!");
	}
