...
/**
 * A {@link org.parboiled.trees.TreeNode} specialiation that allow for mutability of the tree structure. The three defined methods are all expected to properly uphold the trees "linking back" contract, where children have their parent fields point to the node actually holding them in their children list at all times.
 * The three defined methods are the basic ones required, other convenience methods (like a simple addChild(child) without index) are defined as static methods of the {@link org.parboiled.trees.TreeUtils} class.
 *
 * @param <T>
 *            the actual implementation type of this TreeNode
 */
public interface MutableTreeNode<T extends MutableTreeNode<T>> extends TreeNode<T> {
	 ...
	 *
	 * @param index
	 *            the index under which to set this child into the children list
	 * @param child
