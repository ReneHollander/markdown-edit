...
/**
 * A {@link ParseRunner} implementation that is able to recover from {@link org.parboiled.errors.InvalidInputError}s in the input and therefore report more than just the first {@link org.parboiled.errors.InvalidInputError} if the input does not conform to the rule grammar.
 * Error recovery is done by attempting to either delete an error character, insert a potentially missing character or do both at once (which is equivalent to a one char replace) whereby this implementation is able to determine itself which of these options is the best strategy. If the parse error
 * cannot be overcome by either deleting, inserting or replacing one character a resynchronization rule is determined and the parsing process resynchronized, so that parsing can still continue. In this way the RecoveringParseRunner is able to completely parse all input texts (This ParseRunner never
 * returns an unmatched {@link org.parboiled.support.ParsingResult}). If the input is error free this {@link ParseRunner} implementation will only perform one parsing run, with the same speed as the {@link BasicParseRunner}. However, if there are
 * {@link org.parboiled.errors.InvalidInputError}s in the input potentially many more runs are performed to properly report all errors and test the various recovery strategies.
 */
public class RecoveringParseRunner<V> extends AbstractParseRunner<V> {
	...
	/**
	 * Create a new RecoveringParseRunner instance with the given rule and input text and returns the result of its {@link #run(String)} method invocation.
	 *
	 * @param rule
	 *            the parser rule to run
	 * @param input
	...
	public ParsingResult<V> run(InputBuffer inputBuffer) {
		...
		ParseRunner<V> basicRunner = new BasicParseRunner<V>(getRootMatcher()).withParseErrors(getParseErrors()).withValueStack(getValueStack());
	...
	private boolean performLocatingRun(InputBuffer inputBuffer) {
		resetValueStack();
		ParseRunner<V> locatingRunner = new ErrorLocatingParseRunner<V>(rootMatcherWithoutPTB, getInnerHandler()).withParseErrors(getParseErrors()).withValueStack(getValueStack());
		lastParsingResult = locatingRunner.run(inputBuffer);
		errorIndex = lastParsingResult.matched ? -1 : getParseErrors().remove(getParseErrors().size() - 1).getStartIndex();
		return lastParsingResult.matched;
	}

	private void performReportingRun() {
		resetValueStack();
		ParseRunner<V> reportingRunner = new ErrorReportingParseRunner<V>(rootMatcherWithoutPTB, errorIndex, getInnerHandler()).withParseErrors(getParseErrors()).withValueStack(getValueStack());
	...
	/**
	 * A {@link org.parboiled.MatchHandler} implementation that recognizes the special {@link org.parboiled.support.Chars#RESYNC} character to overcome {@link org.parboiled.errors.InvalidInputError}s at the respective error indices.
	 */
	...
	/**
	 * This MatcherVisitor collects the minimal set of actions that has to run underneath a resyncronization sequence in order to maintain a consistent Value Stack state.
	 */
