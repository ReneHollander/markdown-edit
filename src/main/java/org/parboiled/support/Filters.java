...
public class Filters {
	...
	public static final Predicate<Node<Object>> SKIP_EMPTY_OPTS = new Predicate<Node<Object>>() {
		public boolean apply(Node<Object> node) {
			return hasChildren(node) || node.getEndIndex() != node.getStartIndex() || !"Optional".equals(node.getLabel());
		}
	};
	...
	public static final Predicate<Node<Object>> SKIP_EMPTY_ZOMS = new Predicate<Node<Object>>() {
		public boolean apply(Node<Object> node) {
			return hasChildren(node) || node.getEndIndex() != node.getStartIndex() || !"ZeroOrMore".equals(node.getLabel());
		}
	};
	...
	/**
	 * A predicate usable as a filter (element) of a {@link org.parboiled.parserunners.TracingParseRunner}. Enables printing of rule tracing log messages for all input in the given range of input lines.
	 *
	 * @param firstLine
	...
	/**
	 * A predicate usable as a filter (element) of a {@link org.parboiled.parserunners.TracingParseRunner}. Enables printing of rule tracing log messages for all given rules and their sub rules.
	 *
	 * @param rules
	...
	/**
	 * A predicate usable as a filter (element) of a {@link org.parboiled.parserunners.TracingParseRunner}. Enables printing of rule tracing log messages for all sub rules of the given rules.
	 *
	 * @param rules
		...
		return new Predicate<Tuple2<Context<?>, Boolean>>() {
			public boolean apply(Tuple2<Context<?>, Boolean> tuple) {
				...
				for (Rule rule : rules) {
					...
					if (tuple.a.getMatcher() != matcher && path.contains(matcher))
						return true;
				}
				return false;
			}
	...
	/**
	 * A predicate usable as a filter (element) of a {@link org.parboiled.parserunners.TracingParseRunner}. Enables printing of rule tracing log messages for all matched rules.
	 *
	 * @return a predicate
	 */
